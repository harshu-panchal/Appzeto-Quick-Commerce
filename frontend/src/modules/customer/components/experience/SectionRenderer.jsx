...
const BannerCarousel = ({ section, items }) => {
  ...
  React.useEffect(() => {
    ...
    // Loop through slides safely instead of letting the index grow unbounded
    ...
    <div className="overflow-hidden">
      ...
      >
        {items.map((b, idx) => (
          <div
            key={idx}
            className="min-w-full md:min-w-[320px] h-[190px] bg-slate-100 rounded-3xl relative overflow-hidden"
          >
            <img
              src={b.imageUrl}
              alt={b.title || section.title || "Banner"}
              className="w-full h-full object-cover"
            />
            {(section.title || b.title || b.subtitle) && (
              <div className="absolute inset-0 bg-linear-to-t from-black/50 via-black/10 to-transparent flex items-end p-4">
                <div>
                  <div className="text-xs font-black text-white/80 uppercase tracking-[0.2em]">
                    {section.title}
                  </div>
                  {b.title && (
    ...
    <div className="space-y-8">
      {sections.map((section) => {
        ...
        if (section.displayType === "banners") {
          ...
          if (!items.length) return null;
          return <BannerCarousel key={section._id} section={section} items={items} />;
        }
              ...
              <div className="rounded-3xl bg-white shadow-[0_18px_45px_rgba(15,23,42,0.06)] border border-slate-100 px-3.5 py-3">
                <div className="grid grid-cols-4 gap-3">
                  {items.map((cat) => (
                    ...
                    >
                      <div className="relative aspect-square w-full rounded-2xl bg-[#F8F9FA] border border-slate-100/80 flex items-center justify-center overflow-hidden transition-all duration-200 group-hover:border-[#0c831f]/40 group-hover:bg-white group-hover:shadow-[0_10px_25px_rgba(15,23,42,0.08)]">
                        {cat.image ? (
                          <img
                            src={cat.image}
                            alt={cat.name}
                            className="w-4/5 h-4/5 object-contain mix-blend-multiply transition-transform duration-200 group-hover:scale-105"
                          />
                        ) : (
              ...
              >
                {items.map((product) => (
                  <div key={product._id || product.id}>
                    <ProductCard product={product} compact={columns > 2} />
                  </div>
                ))}
              </div>
            </div>
