        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Monitor all active delivery assignments in real-time.</p>
                </div>
            ...
            <Card className="overflow-hidden border-slate-200 shadow-sm bg-white">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Delivery Boy</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">Route</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Customer</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Status</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-right">Last Update</th>
                            </tr>
                        </thead>
                            ...
                            {filteredFleet.map((item) => (
                                <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-6 py-4">
                                        <span className="text-sm font-bold text-slate-900 leading-none">{item.id}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <button
                                        ...
                                        >
                                            <p className="text-sm font-bold text-slate-900 group-hover:text-primary transition-colors underline decoration-dotted underline-offset-4 decoration-slate-300 group-hover:decoration-primary">{item.deliveryBoy.name}</p>
                                            <p className="text-xs text-primary font-medium">{item.deliveryBoy.phone}</p>
                                        </button>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-center gap-3 text-slate-600">
                                            <span className="text-xs font-semibold bg-indigo-50 text-indigo-700 px-2 py-1 rounded border border-indigo-100">{item.seller.name}</span>
                                            <HiOutlineArrowRight className="text-slate-300" />
                                            <span className="text-xs font-semibold bg-emerald-50 text-emerald-700 px-2 py-1 rounded border border-emerald-100">{item.customer.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div>
                                            <p className="text-sm font-bold text-slate-900">{item.customer.name}</p>
                                            <p className="text-xs text-slate-500 font-medium">{item.customer.phone}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <Badge
                                            variant={item.status === 'On the Way' ? 'info' : item.status === 'At Pickup' ? 'warning' : 'primary'}
                ...
                {filteredFleet.length === 0 && (
                    <div className="py-12 text-center text-slate-400">
                        <p className="text-sm font-medium tracking-wide">No active missions matching your search.</p>
                    </div>
                )}
            </Card>
                            ...
                            <div className="pt-16 pb-8 px-4 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4 text-sm">
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Mobile Number</p>
                                    ...
                                    <div className="flex items-center gap-4 text-sm">
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Vehicle Details</p>
