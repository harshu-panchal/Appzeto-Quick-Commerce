...
const FleetRadar = () => {
    ...
    const [fleet] = useState([
        { id: 'R-101', name: 'Amit Kumar', status: 'delivering', battery: 85, order: '#ORD-9921', distance: '1.2km away', coords: { x: 45, y: 35 } },
        { id: 'R-102', name: 'Sunil Singh', status: 'idle', battery: 42, order: null, distance: 'Nearby Hub', coords: { x: 25, y: 65 } },
        { id: 'R-103', name: 'Vikash Pal', status: 'picking', battery: 92, order: '#ORD-9925', distance: '0.5km to shop', coords: { x: 65, y: 25 } },
        { id: 'R-104', name: 'Deepak Raj', status: 'delayed', battery: 12, order: '#ORD-9901', distance: 'Stuck in traffic', coords: { x: 80, y: 70 } },
    ]);
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-1">
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-8">
                        ...
                        <div className="text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Avg Deliv. Time</p>
                            <h4 className="text-xl font-black text-emerald-600">14.2m</h4>
                        </div>
                        <div className="text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">SLA Health</p>
                            ...
                            {fleet.map((r) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        selectedRider?.id === r.id ? "bg-slate-900 text-white shadow-lg" : "hover:bg-slate-50 text-slate-700"
                                    )}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center transition-all",
                                            selectedRider?.id === r.id ? "bg-white/10" : "bg-slate-100/50"
                                        )}>
                                            <HiOutlineTruck className={cn("h-5 w-5", r.status === 'delayed' ? "text-rose-500" : "text-primary")} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <p className="text-[11px] font-black truncate">{r.name}</p>
                                                <Badge
                                                    variant={r.status === 'delivering' ? 'success' : r.status === 'delayed' ? 'danger' : 'secondary'}
                                                    className={cn("text-[7px] font-black uppercase", selectedRider?.id === r.id && "bg-white/20 text-white border-none")}
                                                >
                                                    {r.status}
                                                </Badge>
                                            </div>
                                            <p className={cn("text-[9px] font-bold opacity-60 mt-0.5", selectedRider?.id === r.id ? "text-white" : "text-slate-500")}>
                                                {r.distance}
                                            </p>
                                        </div>
                                    </div>
                ...
                <Card className="flex-1 border-none shadow-2xl ring-1 ring-slate-200 rounded-[40px] overflow-hidden bg-slate-900 relative">
                    ...
                    <div className="relative w-full h-full p-12">
                        {fleet.map((r) => (
                            ...
                            >
                                ...
                                >
                                    <HiOutlineTruck className={cn("h-5 w-5", selectedRider?.id === r.id ? "text-white" : r.status === 'delayed' ? "text-rose-500" : "text-primary/60")} />
                                    {r.status === 'delivering' && (
                                    ...
                                    <Card className="bg-slate-950/90 backdrop-blur-xl border border-white/10 p-6 rounded-[32px] text-white">
                                        <div className="flex items-center justify-between mb-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h4 className="text-sm font-black">{selectedRider.name}</h4>
                                                    <p className="text-[10px] font-bold text-white/40">{selectedRider.id}</p>
                                                </div>
                                            </div>
                                        ...
                                        <div className="space-y-4">
                                            <div className="grid grid-cols-2 gap-3">
                                                <div className="bg-white/5 p-3 rounded-2xl">
                                                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest">Load</p>
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <HiOutlineBolt className={cn("h-3.5 w-3.5", selectedRider.battery < 20 ? "text-rose-500 animate-pulse" : "text-emerald-500")} />
                                                        <span className="text-xs font-black">{selectedRider.battery}% Energy</span>
                                                    </div>
                                                </div>
                                                <div className="bg-white/5 p-3 rounded-2xl">
                                                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest">Status</p>
                                                    <p className="text-xs font-black mt-1 uppercase text-primary tracking-tighter">{selectedRider.status}</p>
                                                </div>
                                            </div>
                                            ...
                                            {selectedRider.order && (
                                                <div
                                                    onClick={() => {
                                                        showToast(`Navigating to ${selectedRider.order} overview...`, 'info');
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <p className="text-[8px] font-black opacity-60 uppercase">Active Order</p>
                                                            <p className="text-xs font-black">{selectedRider.order}</p>
                                                        </div>
                                                    </div>
                    ...
                    <div className="absolute bottom-8 right-8 flex flex-col gap-2">
                        <div className="bg-slate-950/80 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-white/10 flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-white/70 uppercase tracking-widest">Real-time Telemetry Active</span>
                        </div>
                    </div>
                </Card>
            </div>
