...
const Analytics = () => {
    ...
    const stats = [
        { label: 'Total Sales', value: '₹1,24,500', trend: '+12.5%', icon: HiOutlineArrowTrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        { label: 'Total Orders', value: '1,245', trend: '+8.2%', icon: HiOutlineShoppingBag, color: 'text-indigo-600', bg: 'bg-indigo-50' },
        { label: 'Avg Order Value', value: '₹850', trend: '-2.4%', icon: HiOutlineUsers, color: 'text-amber-600', bg: 'bg-amber-50' },
        { label: 'Conversion Rate', value: '3.8%', trend: '+0.5%', icon: HiOutlineChartBar, color: 'text-rose-600', bg: 'bg-rose-50' }
    ];
        ...
        <div className="space-y-8 pb-16">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
                            Advanced Analytics
                            <Badge variant="success" className="text-[9px] px-1.5 py-0 font-bold tracking-wider uppercase bg-emerald-100 text-emerald-700">Real-time Insights</Badge>
                        </h1>
                        <p className="text-slate-500 text-sm mt-0.5 font-medium">Detailed breakdown of your business performance and customer behavior.</p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <div
                            ...
                            className="h-10 w-10 border border-slate-200 rounded-xl flex items-center justify-center bg-white cursor-pointer hover:bg-slate-50 transition-colors shadow-sm"
                        >
                        ...
                        <ShimmerButton
                            ...
                            className="px-5 py-2.5 rounded-xl text-xs font-bold text-white shadow-lg disabled:opacity-50"
                ...
                {stats.map((stat, i) => (
                    <BlurFade key={i} delay={0.1 + (i * 0.05)}>
                        <MagicCard
                            ...
                            gradientColor={stat.bg.includes('emerald') ? "#ecfdf5" : stat.bg.includes('indigo') ? "#eef2ff" : stat.bg.includes('amber') ? "#fffbeb" : "#fff1f2"}
                        >
                            <div className="p-6 relative z-10 flex items-start justify-between">
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{stat.label}</p>
                                    <h4 className="text-2xl font-black text-slate-900 mt-1 tracking-tight">{stat.value}</h4>
                                    <div className={cn(
                                        ...
                                        {stat.trend.startsWith('+') ? <HiOutlineArrowUpRight className="mr-0.5" /> : <HiOutlineArrowDownRight className="mr-0.5" />}
                                        {stat.trend}
                                        <span className="text-slate-400 ml-1 font-medium">vs prev 7d</span>
                                    </div>
                                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                {['Daily', 'Weekly', 'Monthly'].map((range) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            range === 'Daily' ? "bg-white text-primary shadow-sm" : "text-slate-400 hover:text-slate-600"
                                        )}
                                    >
                                        {range}
                                    </button>
                        ...
                        <div className="h-[400px] w-full mt-4">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={MOCK_SALES_DATA} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                    ...
                                    />
                                    <Tooltip
                                        contentStyle={{ backgroundColor: '#fff', borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)' }}
                                        itemStyle={{ fontSize: '11px', fontWeight: 900 }}
                                        cursor={{ stroke: '#3b82f6', strokeWidth: 1, strokeDasharray: '4 4' }}
                                    />
                                    <Area type="monotone" dataKey="sales" stroke="#3b82f6" strokeWidth={4} fillOpacity={1} fill="url(#colorSales)" />
                                    <Area type="monotone" dataKey="traffic" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" fillOpacity={1} fill="url(#colorTraffic)" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    </Card>
                ...
                <BlurFade delay={0.5} className="lg:col-span-1">
                    <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl p-6 bg-white flex flex-col items-center justify-center group h-full">
                        <div className="w-full text-center mb-6">
                            <h3 className="text-lg font-black text-slate-900">Category Mix</h3>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Inventory Distribution</p>
                        </div>
                        <div className="h-[350px] w-full flex items-center justify-center">
                            <ResponsiveContainer width="100%" height="100%">
                                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={MOCK_CATEGORY_DATA}>
                        ...
                        <div className="grid grid-cols-3 gap-2 w-full mt-4">
                            {MOCK_CATEGORY_DATA.slice(0, 3).map((cat, idx) => (
                                <div key={idx} className="bg-slate-50 p-3 rounded-2xl flex flex-col items-center border border-slate-100/50">
                                    <p className="text-[10px] font-black text-slate-900">{cat.A}</p>
                                    <p className="text-[8px] font-bold text-slate-400 uppercase">{cat.subject}</p>
                                </div>
                            ))}
                        </div>
                    </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <BlurFade delay={0.6}>
                    <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl p-0 overflow-hidden bg-white">
                        <div className="p-6 border-b border-slate-50">
                            ...
                            <p className="text-xs text-slate-500 font-medium">Bestsellers by sales volume and revenue generation.</p>
                        </div>
                            ...
                            {MOCK_TOP_PRODUCTS.map((product, i) => (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="h-10 w-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 font-black text-xs group-hover:bg-primary group-hover:text-white transition-all">
                                            {i + 1}
                                        </div>
                                        <div>
                                            <h4 className="text-sm font-black text-slate-900">{product.name}</h4>
                                            <p className="text-[11px] text-slate-400 font-bold">{product.sales} units sold</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <div className={cn(
                                            ...
                                            {product.trend > 0 ? <HiOutlineArrowUpRight className="h-3 w-3 mr-0.5" /> : <HiOutlineArrowDownRight className="h-3 w-3 mr-0.5" />}
                ...
                <BlurFade delay={0.7}>
                    <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl p-6 bg-white overflow-hidden group h-full">
                        <div className="mb-8">
                            <h3 className="text-lg font-black text-slate-900">Customer Acquisition</h3>
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Traffic Origin Analysis</p>
                        </div>
                            ...
                            <div className="w-full md:w-1/2 space-y-4">
                                {MOCK_TRAFFIC_SOURCES.map((source, i) => (
                                    <div key={i} className="flex items-center justify-between">
                                        ...
                                        <span className="text-xs font-black text-slate-900">{(source.value / 10).toFixed(0)}%</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-3 gap-4 border-t border-slate-50 pt-8">
                            <div className="text-center">
                                ...
                                <p className="text-[10px] font-black text-slate-900 tracking-tight">Mumbai</p>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Top City</p>
                            </div>
                            <div className="text-center">
                                <div className="h-10 w-10 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center mx-auto mb-2">
                                    <HiOutlineClock className="h-5 w-5" />
                                </div>
                                <p className="text-[10px] font-black text-slate-900 tracking-tight">8 PM - 10 PM</p>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Peak Time</p>
                            </div>
                            <div className="text-center">
                                ...
                                <p className="text-[10px] font-black text-slate-900 tracking-tight">82% Mobile</p>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Top Device</p>
                            </div>
                        </div>
                    </Card>
                </BlurFade>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Product ID: SKU-928374</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 bg-emerald-50 rounded-2xl">
                                <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Revenue</p>
                                <p className="text-xl font-black text-emerald-900">{selectedProduct.revenue}</p>
                            </div>
                            <div className="p-4 bg-blue-50 rounded-2xl">
                                <p className="text-[10px] font-black text-blue-700 uppercase tracking-widest">Units Sold</p>
                                <p className="text-xl font-black text-blue-900">{selectedProduct.sales}</p>
                            </div>
                        </div>

                        <div className="space-y-2">
                            <p className="text-xs font-black text-slate-900 uppercase tracking-widest pl-1">Sales velocity</p>
                            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div className="h-full bg-primary w-[75%]" />
                            </div>
                            <p className="text-[10px] text-slate-400 font-bold text-right pt-1">+12% faster than last week</p>
                        </div>
            ...
            >
                <div className="space-y-6">
                    <p className="text-sm text-slate-500 font-medium text-center">Generate a secure link to share this dashboard with your team or partners.</p>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {['PDF', 'Excel', 'Image'].map(format => (
                            ...
                            >
                                ...
                                <span className="text-[10px] font-black uppercase text-slate-500 group-hover:text-primary">{format}</span>
                            </button>
                        ))}
                    </div>
                </div>
